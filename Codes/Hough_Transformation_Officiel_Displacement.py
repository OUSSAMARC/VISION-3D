...
while True:
    ...
    # Predict next position based on previous movement

    predicted_position = (prev_position[0] + velocity[0], prev_position[1] + velocity[1])
    ...
    # If detection is valid, update position and velocity
    distance = np.linalg.norm(np.array(max_loc) - np.array(prev_position))

    # If max_loc is too far from predicted position, use the predicted one
    if  25 <distance < 200 and 0 <ct :
            new_position = predicted_position
            prev_position = new_position

    else:
        # If tracking is lost, use predicted position
        new_position = max_loc
        velocity = (new_position[0] - prev_position[0], new_position[1] - prev_position[1]) 
        prev_position = new_position
    

    frame_detected = frame.copy()
    # Define the top-left and bottom-right corners of the rectangle
    top_left = (new_position[0] - h // 2, new_position[1] - w // 2)
    bottom_right = (new_position[0] + h // 2, new_position[1] + w // 2)
